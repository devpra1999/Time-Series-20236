...
plot(dropFirst(NileFilt$m),ylab = "Filtered Estimates")
```

i'm doing this because i don't know if there's any hidden function in this second method
...
As we can see from the graph above, the standard deviations of the filtering state reach their maximal values at the beginning of the time series, while sharply decreasing as time pass by. This reflect the fact that we have less information about the initial state of the system. As new observations arrive the filtering state estimates become more and more accurate, hence the decrease in the standard deviations over time. 


FORECASTING

i've made a few graph honestly i prefer the second one , in the final version you can delete the first

```{r forecast first graph}

plot(window(Nile, start=1871), type="o", ylab="level", cex=.3)
lines(window(outFilt$f, start=1871), type="o", col='orange', cex=.3)

```

also here dropping the first observation would not be a bad idea i post the command here below:
lines(dropFirst(outFilt$m), lty = "longdash", col='darkorange') instead of lines

```{r forecasting}
par(mar = c(2, 4, 1, 1) + 0.1, cex = 0.6)
a <- window(cbind(Nile, outFilt$f),
start = 1871, end = 1970)
plot(a[, 1], type = 'o', col = "black",
xlab = "", ylab = "Level")
lines(a[, 2], lty = "longdash", col='red')
leg <- c("data", paste("one-step-ahead forecast, W/V =0.097"))
legend("bottomleft", legend = leg,
col = c("black", "red"),
lty = c("solid", "longdash", "dotdash"),
pch = c(1, NA, NA), bty = "n")
```
variances here are random numbers i've choosen. As i said i'm not very fond of this part of theory so if you have better guesses on what values may be of interest feel free to change them 
...
plot(Nile,type="o", xlab = "", ylab = "Level",col='black')
...
We can see from the graph that when the increase W relative to V, the signal to noise ratio decreases and the uncertainty in the forecast increases. On the other hand, if V gets larger the signal gets larger relative to the noise and there is less uncertainty in the estiamates.