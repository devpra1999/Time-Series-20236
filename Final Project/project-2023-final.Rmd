...
ggplot(data = world) +
  ...
  geom_sf(data = Stations, size = 3, shape = 23, fill = "darkred") +
    coord_sf(xlim = c(-135, -100), ylim = c(24.5, 55), expand = T)
```
...
Let $(Y_{j,t}, \theta_{j,t})$ denote, respectively, the observed measurement and the signal in station $j$ at time $t$. 
For a single station, let's consider a simple random walk plus noise model 
$$
\begin{cases}
\begin{aligned}
...
Yet, in order to take the spatial dependence into account, we need a multivariate model for the $m$-dimensional $Y_t=(Y_{j_1,t}, \ldots, Y_{j_m,t})'$ - for us, the PM$_{25}$ observeded at stations $j_1, \ldots, j_m$ (take $m=3$ or $4$). We may write a DLM
$$
\begin{cases}
\begin{aligned}
...
\end{bmatrix}
\]
while the evolution errors $w_t = \left(w_{j_1,t}, \dots, w_{j_m,t} \right)$ are spatially correlated; that is, the covariance matrix $W$ is not diagonal and expresses the intuitive idea that the larger the distance between two stations, say $j$ and $k$, the smaller is the covariance $W[j,k]=Cov(w_{j.t}, w_{k,t})$.
...
$$ 
W[i,k] = Cov(w_{j.t}, w_{k,t}) = \sigma^2 \exp(- \phi D[j,k]),  \quad j,k=1, \ldots, m, 
$$
where $\sigma^2>0$; $\phi >0$ is a *decay parameter*; and 
$D[j,k]$ is the distance between stations $j$ and $k$ (see below about computing the $m \times m$ matrix $D$ of the pairwise distances among the stations). Note that this specification assumes that
$V(w_{j,t})=\sigma^2$, constant across locations, and expresses spatial dependence: the $Cov(w_{j.t}, w_{k,t})$ gets smaller, the more far apart are the stations $j$ and $k$.

Our suggestion is that you try this model for the $\text{PM}_{2.5}$, after some elaboration of the data (suggested to simplify your analysis): 
...
- Moroever, to simplify your analysis (smoothing the irregularities of hourly data), work with a coarser scale, e.g. by taking 12-hours averages. 

The resulting time series will be the object of the analysis.

- Look at the data: describe the time series of $\text{PM}_{2.5}$ through appropriate plots. Do the plots suggest spatial dependence?
